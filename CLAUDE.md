...
17. **Data layer unico (TanStack Query)** - ogni lettura Supabase in front e admin passa da `useQuery` via `@thaiakha/shared/query` (`AppQueryProvider` montato in `front/main.tsx` e `admin/main.tsx`; la dipendenza `@tanstack/react-query` vive SOLO in shared). Mai `useEffect + useState + let cancelled` per un fetch. Hook canonici front: `usePageSections` (page_sections), `useMediaAsset`/`useMediaAssets`, `useSiteMetadata` (campi di contorno site_metadata), `usePageMetadata`, `useSEO`, `useFrontHomeCards`, `useAudioAsset`; admin: chiave `['page_metadata','site_metadata_admin',lang,slug]`, `queryClient.clear()` al logout (query RLS-scoped); front: le query con dati dell'utente hanno chiave con prefisso `'user'` (`['user','companion_count',id]`) e `handleLogout` fa `removeQueries({ queryKey: ['user'] })`, il resto e' pubblico e resta. Hook condivisi per le tassonomie: `useContentCategories(domain)`, `useSpicinessLevels`, `useDietaryKnowledge`. `services/_cache.ts` (localStorage SWR) resta solo per i service non ancora dietro TanStack: non aggiungerne di nuovi.
18. **Routing front a tabella** - le route vivono in `front/src/lib/routes.tsx` (`ROUTES`: ids canonico+alias, `hasSlug`, `gate: 'profile'`, `render(ctx)`); `App.tsx` e' un router puro. Nuova pagina = una riga in `ROUTES`, mai un `case`/`if` sugli slug in App.tsx; alias URL vecchi in `LEGACY_SLUG_MAP`, alias per `onNavigate` in `NAV_SLUG_ALIASES`. react-router valutato e NON adottato (routing i18n nel LanguageProvider/SEOHead/Worker).
...
20. **Lint a zero warning** - `pnpm --filter front lint` e `--filter admin lint` sono a 0 warning dal 2026-08-19 e devono restarci: niente `any` nuovi (tipi generati `Tables<'…'>`, tipi di dominio, `unknown` + narrowing; cast unico `as unknown as Row[]` solo dove PostgREST non inferisce le select concatenate, con commento; per i sidecar `*_translations` quel cast vive UNA volta dentro `mergeSidecarRow(s)<T>` in `shared/lib/mergeTranslation.ts`: i call site passano `data` nudo, dichiarano la forma con `<T>` e non scrivono `as unknown as`); deps degli effetti aggiunte solo se stabili, altrimenti `eslint-disable-next-line` con motivo; costanti/helper fuori dai file componente (fast refresh).
