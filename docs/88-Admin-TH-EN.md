  ...
  await i18n
    ...
    .use(resourcesToBackend(async (language: string, namespace: string) => {
      try {
        ...
        console.error(`Failed to load i18n namespace: ${namespace}`, error);
      }
    }))
    .init({
      lng,
...
4. **Sincronizzazione**: Assicurarsi che ogni nuova chiave aggiunta nel file inglese sia presente (anche se non tradotta) nel file Thai per evitare fallback visuali.
