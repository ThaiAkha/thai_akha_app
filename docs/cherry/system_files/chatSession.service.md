# 🗄️ Chat Session & Persistence Service

**Source File:** `packages/shared/src/services/chatSession.service.ts`  
**Description:** The authoritative service for managing conversation persistence, session lifecycle, and rate-limiting. It bridges the frontend experience with the Supabase `chat_sessions` and `chat_messages` tables, ensuring that both guest and registered users have a consistent history across text and voice modes.
...
## 🛡️ Business Logic & Rate Limiting

`checkRateLimit` ora delega a una singola RPC PostgreSQL `check_chat_rate_limit` (atomica, `SECURITY DEFINER`), sostituendo le due query separate precedenti.

**Stato attuale: limiti temporaneamente disabilitati** — la RPC restituisce sempre `allowed=true, reason='limits disabled'`. La logica completa è scritta nel body della funzione SQL, pronta da riattivare rimuovendo le 2 righe di bypass.

**Logica completa (quando riattivata):**
- **VIP**: utente con prenotazione confermata (`booking_date >= CURRENT_DATE`) → illimitato
- **Logged in**: max 30 messaggi/24h contati su `chat_messages`
- **Guest**: max 10 messaggi per sessione (richiede `session_token` passato dal caller — TODO)
- **Fallback**: se la RPC fallisce → `allowed: true` (non blocca l'utente)

**Migration SQL**: `supabase/migrations/20260407000000_check_chat_rate_limit_rpc.sql`

- **Ephemeral Fallback**: In caso di errore Supabase durante `getOrCreateSession`, viene restituita una sessione in-memory (`ephemeral_*`) che non persiste su DB.
