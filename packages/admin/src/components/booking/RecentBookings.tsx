...
export default function RecentBookings() {
    ...
    if (loading) return <div className="p-6 text-center text-sub">{t('recentBookings.loading')}</div>;
            ...
            <div className="max-w-full overflow-x-auto">
                <Table>
                    <TableHeader className="border-gray-100 dark:border-gray-800 border-y">
                        <TableRow>
                            <TableCell isHeader className="py-3 font-medium text-sub text-start text-theme-xs">{t('recentBookings.colUser')}</TableCell>
                            <TableCell isHeader className="py-3 font-medium text-sub text-start text-theme-xs">{t('recentBookings.colDate')}</TableCell>
                            <TableCell isHeader className="py-3 font-medium text-sub text-start text-theme-xs">{t('recentBookings.colClass')}</TableCell>
                            <TableCell isHeader className="py-3 font-medium text-sub text-start text-theme-xs">{t('recentBookings.colPax')}</TableCell>
                            <TableCell isHeader className="py-3 font-medium text-sub text-start text-theme-xs">{t('recentBookings.colStatus')}</TableCell>
                        </TableRow>
                    </TableHeader>
                    <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
                        {bookings.map((booking) => (
                            <TableRow key={booking.internal_id}>
                                ...
                                <TableCell className="py-3 text-sub text-theme-sm max-w-[100px] truncate">
