...
export const DriverHeading: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <span className="text-xl font-bold text-title truncate">{children}</span>
);

/** Centro colonna in caricamento (stesso markup dei vecchi rami `=== null`). */
export const LoadingCenter: React.FC<{ label: string }> = ({ label }) => (
    <div className="p-8 text-center"><SectionTitle className="text-sub">{label}</SectionTitle></div>
);
...
export const ArchiveEmpty: React.FC<{ view: DriverView; pendingLabel: string; archiveLabel: string }> = ({ view, pendingLabel, archiveLabel }) => (
    <div className="h-full flex flex-col items-center justify-center text-sub gap-3 py-20">
        ...
        <SectionTitle className="text-sub">{view === 'active' ? pendingLabel : archiveLabel}</SectionTitle>
    </div>
);
