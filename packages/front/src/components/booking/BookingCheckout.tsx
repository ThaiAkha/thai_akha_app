  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-8">
      ...
      <div className="flex flex-col items-center text-center [margin-bottom:var(--space-fluid-xl)] [padding-top:var(--space-fluid-l)]">
        <Typography variant="h1" className="text-white font-black uppercase tracking-tighter mb-4 text-4xl md:text-6xl drop-shadow-2xl">
        ...
        <Typography variant="paragraphS" className="max-w-2xl opacity-60 leading-relaxed">
          Check availability and reserve your Cooking Class in Chiang Mai. Instant<br />
          Reservation - Secure Payments - 100% Refundable
        </Typography>
      </div>

      {/* Step label */}
      <div className="text-center [margin-bottom:var(--space-fluid-xl)]">
        <Typography variant="microLabel" className="text-white/40 tracking-[0.4em] font-black mb-1 block uppercase">
          04. REVIEW &amp; CONFIRM
        </Typography>
        <Typography variant="h2" className="text-4xl md:text-5xl font-black mb-4">Review Your Journey</Typography>
        <Typography variant="paragraphS" className="opacity-50">Double-check your details before we heat up the wok.</Typography>
      </div>

      {/* Summary bar */}
      <div className="w-full bg-surface-elevated/40 backdrop-blur-md border border-white/5 p-4 rounded-[2.5rem] flex flex-col md:flex-row items-center [gap:var(--space-fluid-m)] [margin-bottom:var(--space-fluid-xl)] shadow-2xl">
        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 [gap:var(--space-fluid-s)]">
          {[
            ...
            <div key={label} className="flex items-center [gap:var(--space-fluid-s)] bg-white/[0.03] p-3 pr-8 rounded-full border border-white/5 hover:bg-white/[0.06] transition-all">
              ...
              <div className="flex flex-col">
                <Typography variant="microLabel" className="opacity-30 text-white">{label}</Typography>
                <Typography variant="h6" as="span" className="font-black text-white uppercase tracking-tight">{value}</Typography>
              </div>
            </div>
          ))}
        </div>
        <button onClick={() => setViewStep('selection')}
          className="rounded-full px-8 h-12 bg-white/[0.05] border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all uppercase text-white flex items-center gap-2 group cursor-pointer">
          <Icon name="edit" size="sm" className="opacity-40 group-hover:opacity-100 transition-opacity" />
          <Typography variant="microLabel" as="span" className="text-white">Modify</Typography>
        </button>
      </div>
