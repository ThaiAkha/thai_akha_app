...
import { cn } from '@thaiakha/shared/lib/utils'; // [Source 336]
...
const Divider: React.FC<DividerProps> = ({
  ...
  // Se c'è una LABEL, il layout è Flexbox con AkhaPixelLine sui lati
  if (label) {
    const sideLineClass = "flex-1 overflow-hidden";

    return (
      <div className={cn('flex items-center w-full [padding-block:var(--space-fluid-m)]', className)}>
        {(labelPosition === 'center' || labelPosition === 'right') && (
          <div className={sideLineClass}>
            <AkhaPixelPattern variant="line_simple" size={6} className="[padding-block:0]" />
          </div>
        )}
  ...
  if (vertical) {
    return (
      <div
        className={cn(
          'inline-block h-auto min-h-[1em] w-px self-stretch [margin-inline:var(--space-fluid-s)]',
          variant === 'gradient'
  ...
  return (
    <hr
      className={cn(
        'w-full [margin-block:var(--space-fluid-m)] border-t-0', // Reset base
        variant === 'gradient'
