...
const VARIANT_STYLES: Record<TypographyVariant, { element: React.ElementType; className: string }> = {
  ...
  caption: { element: 'span', className: "font-sans text-xs md:text-sm italic opacity-100" },
};
...
export const Typography: React.FC<TypographyProps> = ({
  ...
  // titleHighlight è escluso: nessun colore auto, riceve colore via className (es. gradient)
  let appliedColor = color;
  if (!appliedColor && variant !== 'titleHighlight') {
  ...
  return (
    <Tag
      className={cn(
        ...
        appliedColor ? COLOR_STYLES[appliedColor] : undefined,
        className
      )}
      {...props}
    >
