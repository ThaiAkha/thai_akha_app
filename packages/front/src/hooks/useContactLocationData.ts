...
export const contactLocationQueryKey = ['contact_location', 'meeting_points+pickup_zones'] as const;
...
export function useContactLocationData() {
  const query = useQuery({
    queryKey: contactLocationQueryKey,
    queryFn: async () => {
      const [mp, pz] = await Promise.all([
        supabase
          .from('meeting_points')
      ...
      return {
        points: (mp.data ?? []) as unknown as MeetingPoint[],
        zones: (pz.data ?? []) as unknown as PickupZoneRow[],
      };
    },
  });
  return {
