// packages/shared/src/prompts/cherrySystem.ts

import { classData } from '../data';
import { DIETARY_KNOWLEDGE_BASE } from '../data';
...
* **The Safety Shield:** If allergies are mentioned, drop the playfulness. Be reassuring and precise. "I will redesign the recipe for you. We treat allergies with hospital-grade seriousness."
* **The Spice Dial:**
...
`;
  ...
  if (allergies && allergies.length > 0) {
    allergyInstruction = `
### CRITICAL ALLERGY ALERT: ${allergies.join(', ').toUpperCase()}
...
`;
  ...
  // 5. CHAMELEON PROTOCOL — tono dinamico per età e ruolo
  const age = userProfile?.age;
  const role = userProfile?.role;
  const lang = userProfile?.preferred_language || 'en';

  let chameleonBlock = '';
  if (role === 'agency' && lang === 'th') {
    chameleonBlock = `\n### CHAMELEON MODE: B2B AGENCY (Thai)\nใช้ภาษาไทยที่สุภาพและเป็นทางการ ให้ข้อมูลที่แม่นยำ ตรงประเด็น และให้ข้อมูลทางธุรกิจที่เป็นประโยชน์\n`;
  } else if (role === 'agency') {
  ...
  const voiceModeBlock = isVoiceMode
    ? `\n### VOICE MODE ACTIVE\nBrevity is key. Max 30 words per turn. Warm, spoken rhythm. Always end with 'kha'.\n`
    : '';
