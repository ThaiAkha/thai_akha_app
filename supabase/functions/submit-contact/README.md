...
## Segreti
`TURNSTILE_SECRET_KEY` (secret della edge, mai con prefisso `VITE_`). Se manca, la verifica si salta con un avviso nei log: serve a non rompere il form fra il deploy e la creazione delle chiavi. Se Cloudflare non risponde, la richiesta viene **rifiutata**: un form contatti può aspettare, una porta aperta no.
